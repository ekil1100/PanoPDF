...
- 全窗口纵向结构：顶部自定义标题栏与阅读工具栏、中间弹性工作区、底部状态栏；没有营销页最大宽度容器。空白／欢迎状态隐藏阅读工具栏。
- 应用 CSS 最小宽度为 640px，这是实现约束，不代表本轮已验证该尺寸。审查覆盖 800、1440、1920px 宽桌面客户区，不发布移动端。
- 自定义标题栏高 40px，左侧“文件”菜单、中间居中文件名、右侧窗口控件；不再额外显示打开文件行。工具栏内边距为 8px 12px，控件组内间距 4px，常规组间横向间距 8px。
...
标题栏是窗口拖动区域；“文件”菜单与宽 46px、高 40px 的窗口按钮使用非拖动区域。窗口状态决定最大化／还原图标与辅助名称；关闭文档与关闭窗口是不同操作。

“文件”按钮展开普通 popover 菜单，提供打开与关闭文档。菜单支持方向键、Home／End、Escape／Tab，退出后合理恢复焦点。首次欢迎页提供打开按钮及三条阅读引导，之后不重复展示。
...
已有空态、加载进度、打开失败、提示条、密码错误、搜索无结果和拖入提示；文案说明问题及恢复动作。浏览器预览明确标记仅本次会话，不能误标为桌面持久化。焦点恢复、菜单键盘操作、窗口控制与启动恢复由 Playwright Electron 测试覆盖；真实标题栏拖动和 Windows／Linux 原生窗口行为仍需实机确认。
